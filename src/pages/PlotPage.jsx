...
export default function PlotPage() {
  ...
  if (!proj || !plot) {
    return (<main className="screen screen--app"><section className="dashboard-page"><TopBar /><div className="empty-state"><p>Parcelle introuvable.</p><button className="cta-primary" onClick={() => navigate('/browse')}>Retour</button></div></section></main>)
  }
    ...
    <main className="screen screen--app">
      <section className="dashboard-page plot-page-skin" style={{ paddingBottom: 'calc(6.5rem + env(safe-area-inset-bottom, 0px))' }}>
        ...
        <div className="detail-map-wrap">
          ...
          <div className="detail-map" style={{ height: '360px' }}><iframe title={`Parcelle ${plot.id}`} src={proj.mapUrl} loading="lazy" allowFullScreen/></div>
        </div>
        ...
        <div className="plot-details-card">
          <div className="plot-detail-row plot-detail-row--header"><div className="plot-detail-id-block"><span className="plot-detail-id">Parcelle #{plot.id}</span><span className="plot-detail-project">{proj.title} · {proj.city}, {proj.region}</span></div><span className="inline-badge plot-inline-badge">{plotStatusLabel(plot.status)}</span></div>
